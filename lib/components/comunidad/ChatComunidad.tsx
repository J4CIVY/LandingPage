...
export default function ChatComunidad({
  ...
  const cargarUsuariosEnLinea = async () => {
    try {
      ...
      // NestJS: GET /community/online-users
      const data = await apiClient.get<{ datos: UsuarioEnLinea[] }>('/community/online-users');
  ...
  const actualizarActividad = async () => {
    try {
      // NestJS: POST /community/online-users
      await apiClient.post('/community/online-users', {});
    } catch (error) {
  ...
  const enviarMensaje = async (e: FormEvent) => {
    ...
    try {
      // NestJS: POST /community/chat
      await apiClient.post('/community/chat', {
