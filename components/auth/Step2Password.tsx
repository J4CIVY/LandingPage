...
export default function Step2Password({ email, onPasswordVerified, onBack }: Step2PasswordProps) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      // Note: RSA encryption may not be needed with JWT
      // Using plain password for now - backend should hash it
      const plainPassword = password;

      // Validar credenciales con backend NestJS
      // NestJS: POST /auth/validate-credentials
      const result = await apiClient.post<{ success: boolean; preAuthToken?: string; message?: string; code?: string; data?: { preAuthToken: string } }>('/auth/validate-credentials', {
        email,
      ...
      const preAuthToken = result.preAuthToken || result.data?.preAuthToken || '';
      onPasswordVerified(preAuthToken);
