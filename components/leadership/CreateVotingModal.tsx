...
const CreateVotingModal: FC<CreateVotingModalProps> = ({
  ...
  const fetchCandidates = async () => {
    try {
      ...
      const result = await apiClient.get<any[]>('/memberships/available-endorsers');
      // Filtrar para mostrar solo candidatos potenciales (no Leaders o Masters actuales)
      const potentialCandidates = result.filter((user: any) => 
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      // NestJS: POST /leadership/voting
      await apiClient.post('/leadership/voting', formData);
      onSuccess();
      onClose();
      resetForm();
