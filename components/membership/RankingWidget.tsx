...
export default function RankingWidget({ 
  ...
  const fetchRealPoints = async () => {
    try {
      ...
      const result = await apiClient.get<{ stats: { totalPoints: number } }>('/users/gamification');
      if (result.stats) {
  ...
  const fetchLeaderboard = async () => {
    ...
    try {
      ...
      const data = await apiClient.get<{ members: LeaderboardMember[] }>('/memberships/leaderboard?limit=5');
