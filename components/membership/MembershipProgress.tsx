...
export default function MembershipProgress({ 
  ...
  const fetchLatestProgress = async () => {
    ...
    try {
      ...
      const data = await apiClient.get<{ membership: { progress: MembershipProgressType } }>('/memberships/me');
      if (data.membership?.progress) {
        setProgress(data.membership.progress);
      }
    } catch (error) {
