...
export default function UpgradeFlowModal({ 
  ...
  const handleUpgradeRequest = async () => {
    ...
    try {
      // NestJS: POST /memberships/request-upgrade
      const data = await apiClient.post<{ allowed: boolean; missingRequirements?: string[]; newMembershipType?: string }>(
        '/memberships/request-upgrade',
        {
      ...
      if (data.newMembershipType && onUpgradeSuccess) {
        setTimeout(() => {
          onUpgradeSuccess();
        }, 2000);
      }
