      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        {items.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">No se encontraron elementos para los filtros seleccionados.</p>
          </div>
        ) : (
                  ...
                  <div key={item.id} className="relative flex flex-col md:flex-row md:items-start space-y-4 md:space-y-0 md:space-x-6">
                    ...
                    <div className="md:hidden bg-gray-50 rounded-lg p-3">
                      <div className="text-center">
                        <div className="text-xl font-bold text-gray-900">{date.day}</div>
                        <div className="text-sm text-gray-600">{date.month} {date.year}</div>
                    ...
                    <div className="hidden md:block flex-shrink-0 w-20 text-right">
                      <div className="text-sm font-medium text-gray-900">{date.day}</div>
                      ...
                      {item.detalles && (
                        <div className="mt-3 text-sm text-gray-600">
                          ...
                          {item.tipo === 'Beneficio' && item.detalles.valorDescuento && (
                            <div>Ahorro: <span className="font-medium text-green-600">${item.detalles.valorDescuento.toLocaleString()}</span></div>
                          )}
                          {item.tipo === 'Reconocimiento' && item.detalles.puntos && (
                            <div>Puntos: <span className="font-medium text-purple-600">{item.detalles.puntos}</span></div>
                          )}
                        </div>
                      )}
                    </div>
