  ...
  useEffect(() => {
    ...
    const verifyEmail = async () => {
      try {
        ...
        const result = await apiClient.get(`/auth/verify-email/${token}`) as {
          message?: string;
          data?: {
            email: string;
            firstName?: string;
        ...
        setVerificationResult({
          success: false,
          message: err.response?.data?.message || 'Error de conexión. Intenta nuevamente más tarde.'
        });
      } finally {
        setIsLoading(false);
      }
...
const ResendVerificationSection: FC = () => {
  ...
  const handleResend = async (e: FormEvent) => {
    ...
    try {
      const apiClient = (await import('@/lib/api-client')).default;
      const result = await apiClient.post('/auth/resend-verification', { email }) as { message?: string };
      setResendMessage(result.message || 'Correo de verificación enviado');
    } catch (error) {
