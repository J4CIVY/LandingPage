...
export default function AdminUsersPage() {
  ...
  const handleToggleUserStatus = async (userId: string, currentStatus: boolean) => {
    try {
      const apiClient = (await import('@/lib/api-client')).default;
      await apiClient.patch(`/admin/users/${userId}/toggle-status`, { isActive: !currentStatus });
  ...
  const handleBulkAction = async (action: string) => {
    ...
    try {
      const apiClient = (await import('@/lib/api-client')).default;
      await apiClient.patch('/admin/users/bulk', { 
  ...
  return (
    <AdminLayout title="Gestión de Usuarios" description="Administrar usuarios registrados">
      {/* Header */}
      <div className="bg-white dark:bg-slate-800 shadow-sm border-b border-gray-200 dark:border-gray-700 mb-6">
        <div className="px-4 lg:px-6 py-4">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div className="flex items-center">
              <Link href="/admin" className="mr-3 lg:mr-4">
                <FaArrowLeft className="text-lg lg:text-xl text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white" />
              </Link>
              <FaUsers className="text-xl lg:text-2xl text-blue-600 mr-3" />
              <div>
                <h1 className="text-lg lg:text-2xl font-bold text-gray-900 dark:text-white">Gestión de Usuarios</h1>
                <p className="text-xs lg:text-sm text-gray-600 dark:text-gray-400 hidden sm:block">Administrar usuarios registrados</p>
              </div>
            </div>
        ...
        {/* Filtros y Búsqueda */}
        <div className="bg-white dark:bg-slate-800 rounded-lg shadow mb-6 p-4 lg:p-6">
          ...
          {selectedUsers.length > 0 && (
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
        ...
        {/* Paginación */}
        {totalPages > 1 && (
          <div className="bg-white dark:bg-slate-800 px-4 py-3 border-t border-gray-200 dark:border-gray-700 sm:px-6 rounded-b-lg">
            <div className="flex items-center justify-between">
              <div className="flex-1 flex justify-between sm:hidden">
                <button
                  onClick={() => setCurrentPage(Math.max(1, currentPage - 1))}
                  disabled={currentPage === 1}
                  className="relative inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 text-sm font-medium rounded-md text-gray-700 dark:text-gray-300 bg-white dark:bg-slate-800 hover:bg-gray-50 dark:hover:bg-slate-700 disabled:opacity-50"
                >
                  Anterior
                </button>
                <button
                  ...
                  className="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 text-sm font-medium rounded-md text-gray-700 dark:text-gray-300 bg-white dark:bg-slate-800 hover:bg-gray-50 dark:hover:bg-slate-700 disabled:opacity-50"
                >
                  Siguiente
                </button>
              </div>
