...
export default function ProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const data = await apiClient.get('/users/me') as { user?: unknown };
  ...
  const handleSave = async (data: any) => {
    try {
      const apiClient = (await import('@/lib/api-client')).default;
      await apiClient.put('/users/me', data as Record<string, unknown>);
  ...
  const handleAvatarChange = async (file: File) => {
    ...
    try {
      const apiClient = (await import('@/lib/api-client')).default;
      const result = await apiClient.upload('/uploads/profile-image', formData) as { url?: string };
