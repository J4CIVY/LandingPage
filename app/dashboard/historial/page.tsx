...
export default function HistorialPage() {
  ...
  const fetchHistorial = async () => {
    try {
      ...
      const data = await apiClient.get('/users/activity?limit=100') as {
      ...
      const activities = data.activities || [];
        ...
        const historialData: HistorialItem[] = activities.map((activity: any) => {
          ...
          let tipo: 'Evento' | 'Membresía' | 'PQRSDF' | 'Beneficio' | 'Reconocimiento' = 'Evento';
          ...
          // Mapear estados
          let estado: 'completado' | 'pendiente' | 'activo' | 'cerrado' | 'vencido' | 'rechazado' = 'completado';
          if (activity.status === 'pending') estado = 'pendiente';
          else if (activity.status === 'cancelled') estado = 'cerrado';
          else if (activity.status === 'failed') estado = 'rechazado';
          else if (activity.status === 'completed') estado = 'completado';
          else if (activity.status === 'active') estado = 'activo';
          else if (activity.status === 'expired') estado = 'vencido';
