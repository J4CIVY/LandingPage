...
export default function RankingPage() {
  ...
  const fetchRealPoints = async () => {
    try {
      ...
      const result = await apiClient.get('/users/gamification') as {
        stats?: { totalPoints: number };
      };
      if (result.stats) {
        setRealPoints(result.stats.totalPoints);
      }
    } catch (error) {
  ...
  const fetchFullLeaderboard = async () => {
    ...
    try {
      const apiClient = (await import('@/lib/api-client')).default;
      const data = await apiClient.get('/memberships/leaderboard?limit=100') as {
  ...
  const fetchUserRanking = async () => {
    try {
      const apiClient = (await import('@/lib/api-client')).default;
      const data = await apiClient.get('/memberships/me') as {
        ranking?: UserRanking;
      };
      if (data.ranking) {
            ...
            <div className="flex items-center justify-between text-white">
              <div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <p className="text-2xl font-bold">{String(user?.firstName || '')} {String(user?.lastName || '')}</p>
