...
export default function ComunidadPage() {
  ...
  const cargarPublicaciones = async () => {
    try {
      ...
      const url = grupoSeleccionado 
        ? `/community/posts?grupoId=${grupoSeleccionado}`
        : '/community/posts';
      
      const data = await apiClient.get(url) as { datos?: unknown };
      setPublicaciones(Array.isArray(data.datos) ? data.datos as Publicacion[] : []);
  ...
  const cargarGrupos = async () => {
    try {
      ...
      const apiClient = (await import('@/lib/api-client')).default;
      const data = await apiClient.get('/community/groups') as { datos?: GrupoInteres[] };
  ...
  const cargarRanking = async () => {
    try {
      const apiClient = (await import('@/lib/api-client')).default;
      const data = await apiClient.get('/community/ranking') as { datos?: { ranking?: UsuarioRanking[] } };
  ...
  const cargarMensajes = async () => {
    try {
      const apiClient = (await import('@/lib/api-client')).default;
      const data = await apiClient.get('/community/chat') as { datos?: { mensajes?: ChatMensaje[] } };
              ...
              {estadoPublicaciones.cargando ? (
                ...
                publicaciones.map((publicacion) => (
                  <PublicacionCard
                    ...
                    usuarioActual={user as IUser | null}
